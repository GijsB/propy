...
def test_optimization_min_rotation_speed() -> None:
    ...
    prop = WageningenBPropeller(
        ...
        objective=lambda p: p.losses(**wp),
        constraints=[
            lambda p: p.torque_margin(**wp, torque_max=1667435)
        ]
    )
...
def test_torque_limit() -> None:
    ...
    prop = WageningenBPropeller(
        ...
        objective=lambda p: p.losses(**wp),
        constraints=[
            lambda p: p.torque_margin(**wp, torque_max=60)
        ]
    )

    pp = prop.find_performance(**wp)

    assert prop.torque_margin(**wp, torque_max=60) > -5e-8
...
def test_rpm_limit() -> None:
    ...
    prop = WageningenBPropeller(
        ...
        objective=lambda p: p.losses(**wp),
        constraints=[
            lambda p: p.rotation_speed_margin(**wp, rotation_speed_max=20)
        ]
    )

    pp = prop.find_performance(**wp)

    assert prop.rotation_speed_margin(**wp, rotation_speed_max=20) > -1-15
...
def test_tip_speed_limit() -> None:
    ...
    prop = WageningenBPropeller(
        ...
        objective=lambda p: p.losses(**wp),
        constraints=[
            lambda p: p.tip_speed_margin(**wp, tip_speed_max=24)
        ]
    )
    ...
    # That's not really close, weird
    assert prop.tip_speed_margin(**wp, tip_speed_max=24) > -1e-6
